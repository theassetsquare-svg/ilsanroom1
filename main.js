...
(function () {
  ...
  const pages = [
    { url: '/', title: '일산룸 — 예약 전에 반드시 읽어야 할 글', kw: '일산룸 예약 예산 권역 라페스타 웨스턴돔 장항동 일산 룸 저녁 모임' },
    { url: '/guide', title: '일산룸 초보자 가이드 — 처음 가는 사람을 위한 완벽 정리', kw: '초보 가이드 복장 매너 예약 방법 처음 일산룸 이용법' },
    { url: '/review', title: '일산룸 권역별 솔직 후기 — 라페스타 vs 웨스턴돔 vs 장항동', kw: '후기 리뷰 비교 라페스타 웨스턴돔 장항동 일산룸 추천' },
    { url: '/legal', title: '운영 정책 — 19+ · 합법 운영', kw: '운영 정책 19세 합법 정보통신망법 청소년보호법 방심위' }
  ];
...
(function () {
  ...
  if (!wrap) return;
  const qs = [
    { q: '모임의 주요 목적은?', opts: [{ t: '친구들과 가볍게 한잔', v: 'c' }, { t: '거래처 미팅이나 중요 모임', v: 'p' }, { t: '조용히 프라이빗하게', v: 'r' }] },
    { q: '선호하는 분위기는?', opts: [{ t: '활기차고 북적이는 곳', v: 'c' }, { t: '세련되고 고급스러운 곳', v: 'p' }, { t: '아는 사람만 아는 숨은 곳', v: 'r' }] },
    { q: '예산은 어느 정도?', opts: [{ t: '부담 적은 중급선', v: 'c' }, { t: '중상급 ~ 상급선', v: 'p' }, { t: '예산보다 만족도가 우선', v: 'r' }] }
  ];
  const res = {
    c: { tag: '라페스타 추천', title: '라페스타 캐주얼파', desc: '가성비 좋고 접근성 최고인 라페스타가 딱 맞는다. 처음 가는 사람에게도 부담 없고, 다음 일정 연계도 쉬워서 자연스럽게 이용할 수 있다.' },
    p: { tag: '웨스턴돔 추천', title: '웨스턴돔 프리미엄파', desc: '격이 다른 인테리어와 세심한 서비스를 원한다면 웨스턴돔이다. 비즈니스 미팅 만족도가 높고, 한 번 가면 다시 찾게 되는 곳.' },
    r: { tag: '장항동 추천', title: '장항동 프라이빗파', desc: '남의 눈 신경 쓰지 않고 내 취향대로 즐기고 싶다면 장항동이다. 단골이 되면 맞춤 서비스의 진가를 경험하게 된다.' }
  };
  let step = 0, answers = [];
  ...
  function render() {
    if (step < qs.length) {
      const q = qs[step];
      body.innerHTML = '<p class="quiz-q">Q' + (step + 1) + '. ' + q.q + '</p><div class="quiz-opts">' + q.opts.map(o => '<button type="button" class="quiz-opt" data-v="' + o.v + '">' + o.t + '</button>').join('') + '</div>';
      ...
      const r = res[w];
      body.innerHTML = '<div class="quiz-result"><div class="quiz-result-tag">' + r.tag + '</div><h4>' + r.title + '</h4><p>' + r.desc + '</p><a href="tel:01041175556" class="cta-btn" style="display:inline-flex;padding:12px 28px;margin-bottom:12px">전화 예약 010-4117-5556</a><br><button type="button" class="quiz-retry">다시 해보기</button></div>';
...
(function () {
  ...
  const slides = [
    ...
    { day: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=600&q=80&auto=format', night: 'https://images.unsplash.com/photo-1470337458703-46ad1756a187?w=600&q=80&auto=format', cap: '웨스턴돔 매장 — 일반 사무 공간과 프리미엄 라운지, 같은 건물 다른 세계' },
    ...
    { day: 'https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=600&q=80&auto=format', night: 'https://images.unsplash.com/photo-1514933651103-005eec06c04b?w=600&q=80&auto=format', cap: '기본 세팅 — 점심엔 레스토랑, 저녁엔 분위기 있는 라운지로 완전히 달라진다' },
    ...
    { day: 'https://images.unsplash.com/photo-1517457373958-b7bdd4587205?w=600&q=80&auto=format', night: 'https://images.unsplash.com/photo-1560624052-449f5ddf0c31?w=600&q=80&auto=format', cap: '음료 세팅 — 낮 커피 한 잔 예산이면 밤에는 프리미엄 세팅을 받을 수 있다' }
  ];
  ...
  function render() {
    ...
    dots.innerHTML = slides.map((_, i) => '<button type="button" class="gallery-dot' + (i === idx ? ' active' : '') + '" data-i="' + i + '" aria-label="슬라이드 ' + (i + 1) + '"></button>').join('');
